...
popping_email as p
 ...
 LEFT JOIN (select lead.id, lead.popping_email_id, count(lead.id) as nol, sum(popping_email.price) as lc from lead INNER JOIN popping_email on popping_email.id = lead.popping_email_id where lead.status != 'close' and lead.status != 'filtered' and lead.created_at > '$last_7_days' group by lead.popping_email_id ) l on (l.popping_email_id = p.id)

LEFT JOIN (select invoice_head.id, invoice_head.user_id, invoice_head.popping_email_id,  sum(invoice_head.total_cost) as tc, count(invoice_head.id) as noi from invoice_head where invoice_head.status != 'cancel' and invoice_head.created_at > '$last_7_days' group by popping_email_id ) i on (i.popping_email_id = p.id)
